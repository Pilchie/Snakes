...
public partial class Play : IAsyncDisposable
{
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        ...
        await JSRuntime.InvokeAsync<object>("initGame", objRef);
        if (_width <= 0 || _height <= 0)
        {
            throw new InvalidOperationException($"Width: {_width}, Height: {_height}");
        }
    ...
    private async ValueTask Render()
    {
        ...
        {
            throw new InvalidOperationException($"boardWidth: {_boardSize.Width}, boardHeight: {_boardSize.Height}");
        }
    ...
    [JSInvokable]
    public ValueTask OnResize(int width, int height)
    {
        _width = width;
        _height = height;
